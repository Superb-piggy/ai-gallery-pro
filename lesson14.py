...
with col_gen:
    ...
    if uploaded_file is not None:
        # use_container_width=True 让图片自适应宽度
        st.image(uploaded_file, caption="已上传参考图", use_container_width=True)
    ...
    if st.button("🎨 立即生成", type="primary", use_container_width=True):
        if not final_prompt:
            ...
            with st.spinner("🚀 AI 正在绘图..."):
                try:
                    temp_file_path = image_manager.save_uploaded_file(uploaded_file)
                    payload = {"text": final_prompt, "temp_path": temp_file_path}

                    response = requests.post("http://127.0.0.1:8080/image_2_image", json=payload)
                    ...
                    st.image(response.json()['data'], use_container_width=True)
                except Exception as e:
                    st.error(f"连接后端失败: {e}")
